...
- Luna added opt-in raw-window vs legacy-detrended measured-surface loss
  candidates per half and top-level, explicitly labeling detrended_legacy as
  still selected; no physics selection change. Added closed/open/raw-preserve/
  JSON-safe tests. 3 new tests + 29 existing core surface tests passed; focused
  result serialization check clean. No FEM/API/config, no escalation.
