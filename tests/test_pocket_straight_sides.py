...
answers it by making the pocket the UNFILLETED magnet outline with its two side
edges run straight on to the rotor OD:

    [mp1, mp2, E3, <OD ring stations strictly between E3 and E4>, E4, mp5, mp6]

so the iron face runs mp2 → OD with no intermediate vertex at all.
...
def test_the_three_d_rotor_is_the_same_solid_as_the_two_d_one(gap):
    """`_create_rotor` cuts the SAME extended pocket, so its volume is the 2-D
    iron area × the stack length.  A rectangle left in one of the two paths shows
    up here as a percent-level gap."""
    cq = pytest.importorskip("cadquery")
    motor, polys = _build(dict(GEO_200, magnet_up_gap=gap))
    volume = motor._create_rotor(cq).val().Volume()
...
# ═══════════════════════════════════════════════════════════════════════════
#  (h) magnet_up_gap > 0 is a RECESS, and its cusps must be meshable
# ═══════════════════════════════════════════════════════════════════════════
#
...
#
# Why it refused: the magnet's corner fillet arrived at the straight pocket wall
# TANGENTIALLY, and the few-µm cusp that leaves is finer than gmsh's boolean
# tolerance, so the mechanical mesh came out with 22 triangles of 6e-19 mm² at
# 0.00°.  With those in it every contact pair closed to 2e-16 µm, the rotor
# deformed 0.05 µm instead of 6.2, the bore reacted 0.5 % of the applied torque
# and `runaway_verdict` called the machine a runaway.
#
# `_open_fillet_at_top` already existed to blunt exactly that cusp (2026-09-07)
# — it just never fired here, because its "on the top circle" band was a flat
# 0.06 mm: right on the Ø200 it was measured on, five times the Ø50's own weld
# tolerance, and wide enough to swallow the 0.2 mm fillet's own vertices.  The
# band is now 1.2 × the weld (0.06 mm on the Ø200, unchanged to the bit).

#: The Ø50 the defect was found on, pinned field by field like GEO_200.
GEO_50 = {
...
#: was there before.  Checked against `git show 2401d16:` when this landed.
_ZERO_GAP_ROTOR_SHA = "cdd109485f291410f4aff29ce4b6f89310590966"


def test_the_zero_gap_pocket_is_bit_identical():
    """The Ø200 recipe does not move when the cusp band is scaled."""
    _motor, polys = _build(dict(GEO_200, magnet_up_gap=0.0))
    assert _coord_hash(polys["rotor"]) == _ZERO_GAP_ROTOR_SHA, (
...
def test_magnet_up_gap_is_a_recess_and_the_pocket_still_opens_on_the_rim(gap):
    """What ``magnet_up_gap`` means here: air above the magnet, pocket to the OD.

    Three statements, which together are the mode: the magnet's top sits exactly
    ``gap`` below the rotor surface; the rotor keeps NO iron over it (a point at
    mid-recess height on the pole axis belongs to no solid, so it is air); and
    the pocket still reaches the rim, which is what makes the wedge of the side
    walls the retention.
    """
    motor, polys = _build(dict(GEO_50, magnet_up_gap=gap))
    p = motor.parameters
    rotor_or = float(p["rotor_outer_radius"])
    n = int(p["num_poles"])
    rotor = polys["rotor"]
    assert rotor.geom_type == "Polygon" and rotor.is_valid
    assert len(rotor.interiors) == 1, (
        f"the rotor has {len(rotor.interiors)} interior rings — with the pocket "
        f"open on the rim only the shaft bore is a hole; a second one means "
        f"iron closed over a magnet")

    tops = [max(math.hypot(x, y) for x, y in mp.exterior.coords)
            for mp, _ in polys["magnets"]]
    assert len(tops) == n
    assert min(tops) == pytest.approx(rotor_or - gap, abs=1e-6)
    assert max(tops) == pytest.approx(rotor_or - gap, abs=1e-6)
    ...
    for i in range(n):
        ...
        assert not rotor.covers(q), (
            f"pole {i}: iron at r = {r_mid:.3f} mm over the magnet — "
            f"magnet_up_gap is a recess, not a bridge")
        assert not mags.covers(q)
        assert polys["in_band"].covers(q), (
            f"pole {i}: the recess over the magnet belongs to no body at all — "
            f"a void, not air")


@pytest.mark.parametrize("gap", RECESS_GAPS)
def test_the_fillet_meets_the_pocket_wall_at_a_finite_angle(gap):
    """The cusp the mesher chokes on: the fillet must NOT arrive tangentially.

    Measured on the magnet outline itself — the first chord after the junction
    with the long straight side edge has to leave that edge by a real angle.
    Before the band was scaled to the machine this was 9.9° on the Ø50 by
    construction of `_open_fillet_at_top`'s own rule and, worse, the walk broke
    before dropping anything, so the built chord left the wall by ~0°.
    """
    motor, polys = _build(dict(GEO_50, magnet_up_gap=gap))
    worst = 180.0
    for mp, _pol in polys["magnets"]:
        ...
        for i in range(n):
            # a junction is a SHORT chord leaving a LONG straight edge
            if seg[i - 1] < 4.0 * seg[i]:
                continue
            e_in = P[i] - P[i - 1]
            e_out = P[(i + 1) % n] - P[i]
            c = float(e_in @ e_out) / (np.linalg.norm(e_in) * np.linalg.norm(e_out))
            worst = min(worst, math.degrees(math.acos(max(-1.0, min(1.0, c)))))
    assert worst >= 8.0, (
        f"a magnet corner fillet leaves the pocket wall at {worst:.2f}° — that "
        f"is the tangential arrival whose cusp the mesher turns into 0.00° "
...
def test_the_mechanical_mesh_has_no_sub_degree_element(hole, gap):
    ...
    0.00° (rotor_hole 1, magnet_up_gap 0.1).  A zero-area element has a singular
    element stiffness matrix, and a handful of them froze the contact solve.
    """
    from motor_ai_sim.simulation.mechanical.rotor_stress import build_rotor_mesh

    _motor, polys = _build(dict(GEO_50, rotor_hole=hole, magnet_up_gap=gap))
    mesh = build_rotor_mesh(polys, mesh_size_mm=1.5).mesh
    x, y = mesh.p[0, mesh.t] * 1e3, mesh.p[1, mesh.t] * 1e3      # m -> mm

    def _ang(a, b, c):
        v1 = np.stack([x[b] - x[a], y[b] - y[a]])
        v2 = np.stack([x[c] - x[a], y[c] - y[a]])
        cs = (v1 * v2).sum(0) / np.maximum(
            np.linalg.norm(v1, axis=0) * np.linalg.norm(v2, axis=0), 1e-30)
        return np.degrees(np.arccos(np.clip(cs, -1.0, 1.0)))

    amin = np.minimum(np.minimum(_ang(0, 1, 2), _ang(1, 0, 2)), _ang(2, 0, 1))
    area = 0.5 * np.abs((x[1] - x[0]) * (y[2] - y[0])
                        - (x[2] - x[0]) * (y[1] - y[0]))
    assert amin.min() > 1.0, (
        f"rotor_hole {hole}, magnet_up_gap {gap}: {int((amin < 1).sum())} "
        f"element(s) under 1° in the mechanical mesh, worst {amin.min():.4f}°, "
        f"smallest area {area.min():.3g} mm²")
    assert area.min() > 1e-6, (
        f"rotor_hole {hole}, magnet_up_gap {gap}: smallest element "
        f"{area.min():.3g} mm² — a collinear triple survived")
